...
def generate_random_walk(g, start_nodes, length, multi_length, restart_prob, Q = None):
    ...
    if (len(start_nodes) == 1):
        paces = [dgl.sampling.random_walk(g, start_nodes, length = length, restart_prob = prob_tensor)[0][:, 1:] for _ in range(multi_times)]
    else:
        paces = [dgl.sampling.random_walk(g, start_nodes, length = length, restart_prob = prob_tensor)[0][:, 1:] for _ in range(multi_times)]
...
def generate_random_walk_multiThread_high_level(g, start_nodes_block, paces_block, length, multi_length, restart_prob, Q = None):
    """
    ...
    # for i in range(len(start_nodes)):
        ...
        while len(pace) < length - 1:
            ...
            if False:
                ...
                pace = [dgl.sampling.random_walk(g, [start], length = length, restart_prob = prob_tensor)[0] for _ in range(multi_times)]
...
class SLGAD_SubGraphSampling(BaseSubGraphSampling):
    ...
    def __call__(self, g, start_nodes):
        ...
        for i in range(block):
            temp_result = p.apply_async(generate_random_walk_multiThread_high_level, args = (g, start_nodes_block[i], paces_block[i], self.length, 5, 0.9, Q, ))
            # print(Q.get())

...
def get_parse():
    parser = argparse.ArgumentParser(description='Deep Anomaly Detection on Attributed Networks')
...
def train_step(args,model,optimizer,graph,features,label):
    
    model.train()
    optimizer.zero_grad()
    A_hat, X_hat = model(graph, features)
    loss, struct_loss, feat_loss = loss_func(label, A_hat, features, X_hat, args.alpha)
    l = torch.mean(loss)
    l.backward()
    optimizer.step()
...
def test_step(args,model,graph,features,label):
    model.eval()
    A_hat, X_hat = model(graph, features)
    loss, _, _ = loss_func(label, A_hat, features, X_hat, args.alpha)

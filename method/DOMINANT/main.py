...
if __name__ == '__main__':
    ...
    for epoch in range(args.num_epoch):
        loss, struct_loss, feat_loss=train_step(args,model,optimizer,graph,features,adj_label)
        predict_score=test_step(args,model,graph,features,adj_label)
        print("Epoch:", '%04d' % (epoch), "train_loss=", "{:.5f}".format(loss.item()), "train/struct_loss=", "{:.5f}".format(struct_loss.item()),"train/feat_loss=", "{:.5f}".format(feat_loss.item()))
        writer.add_scalars(
            "loss",

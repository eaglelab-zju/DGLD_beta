    ...
    def forward(self, bg, in_feat):
        ...
        with bg.local_scope():
            ...
            for g in unbatchg:
                ...
                # print(g.ndata['feat'])
                # exit()
                anchor_feat = g.ndata['temp'][0, :].clone()
                g.ndata['feat'][0, :] = 0
        ...
        # print(in_feat == bg.ndata["feat"])
        # print((in_feat == bg.ndata["feat"]).sum())
        # exit()
...
class SL_GAD_Model(nn.Module):
    ...
    def forward(self, pos_batchg, pos_in_feat, neg_batchg, neg_in_feat, args):
        ...
        # print(feat_1.shape)
        # exit()
        # return -torch.mean(torch.log(pos_scores_1) + torch.log(1 - neg_scores_1)) / 2, (neg_scores_1 - pos_scores_1 + 1) / 2
